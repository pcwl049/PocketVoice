...
"%CL_PATH%" /std:c++20 /EHsc /W3 /O2 /utf-8 ^
  /I"%ROOT_DIR%\src\pc" ^
  "%ROOT_DIR%\tests\pc\pc_app_controller_test.cpp" ^
  "%ROOT_DIR%\src\pc\pc_app_controller.cpp" ^
  "%ROOT_DIR%\src\pc\pc_runtime.cpp" ^
  /Fe"%BUILD_DIR%\pc_app_controller_test.exe" >nul
if errorlevel 1 exit /b 1

"%BUILD_DIR%\pc_app_controller_test.exe"
exit /b %ERRORLEVEL%
