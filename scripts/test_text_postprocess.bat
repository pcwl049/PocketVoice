...
"%CL_PATH%" /std:c++20 /EHsc /W3 /O2 /utf-8 ^
  ...
  "%ROOT_DIR%\tests\mobile\text_postprocess_test.cpp" ^
  "%ROOT_DIR%\src\mobile\app\src\main\cpp\text_postprocess.cpp" ^
  /Fe"%BUILD_DIR%\text_postprocess_test.exe" >nul
if errorlevel 1 exit /b 1

"%BUILD_DIR%\text_postprocess_test.exe"
exit /b %ERRORLEVEL%
