...
"%CL_PATH%" /std:c++20 /EHsc /W3 /O2 /utf-8 ^
  ...
  "%ROOT_DIR%\tests\mobile\sensevoice_metadata_test.cpp" ^
  "%ROOT_DIR%\src\mobile\app\src\main\cpp\stt_engine.cpp" ^
  /DSTT_ENGINE_METADATA_ONLY ^
  /Fe"%BUILD_DIR%\sensevoice_metadata_test.exe" >nul
if errorlevel 1 exit /b 1

"%BUILD_DIR%\sensevoice_metadata_test.exe"
exit /b %ERRORLEVEL%
