...
"%CL_PATH%" /std:c++20 /EHsc /W3 /O2 /utf-8 ^
  /I"%ROOT_DIR%\src\pc" ^
  "%ROOT_DIR%\tests\pc\audio_sample_converter_test.cpp" ^
  "%ROOT_DIR%\src\pc\audio\sample_converter.cpp" ^
  /Fe"%BUILD_DIR%\audio_sample_converter_test.exe" >nul
if errorlevel 1 exit /b 1

"%BUILD_DIR%\audio_sample_converter_test.exe"
exit /b %ERRORLEVEL%
