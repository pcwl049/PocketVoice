...
"%CL_PATH%" /std:c++20 /EHsc /W3 /O2 /utf-8 ^
  /I"%ROOT_DIR%\src\pc" ^
  "%ROOT_DIR%\tests\pc\pre_roll_buffer_test.cpp" ^
  "%ROOT_DIR%\src\pc\audio\pre_roll_buffer.cpp" ^
  /Fe"%BUILD_DIR%\pre_roll_buffer_test.exe" >nul
if errorlevel 1 exit /b 1

"%BUILD_DIR%\pre_roll_buffer_test.exe"
exit /b %ERRORLEVEL%
