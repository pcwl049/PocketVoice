#!/usr/bin/env bash
# Linux build for the PocketVoice Android app (QNN variant).
# Builds stt_native with STT_USE_QNN=ON, links the ORT libonnxruntime.so
# that was compiled with the QNN Execution Provider (static_lib), and
# bundles the QNN HTP runtime libraries.
#
# Prerequisites (same as CPU build script) plus:
#   - third_party/lib/android/arm64-v8a/libonnxruntime.so MUST be the
#     ORT build with QNN EP (see docs/FIRERED_ASR2_CTC_PROGRESS.md).
#   - QNN runtime .so files placed under:
#       third_party/lib/android/arm64-v8a-qnn/
#         libQnnHtp.so
#         libQnnHtpV79Stub.so
#         libQnnSystem.so
#       (copied from the QAIRT SDK aarch64-android lib dir.)
set -euo pipefail
...
NATIVE_BUILD_DIR="$OUTPUT_DIR/native-qnn"
STAGE_DIR="$OUTPUT_DIR/apk-stage-qnn"
FINAL_APK="$OUTPUT_DIR/PocketVoice-Android-qnn.apk"
QNN_LIB_DIR="$ROOT_DIR/third_party/lib/android/arm64-v8a-qnn"
...
for qlib in libQnnHtp.so libQnnHtpV69Stub.so libQnnHtpV73Stub.so libQnnHtpV75Stub.so libQnnHtpV79Stub.so libQnnHtpV81Stub.so libQnnSystem.so; do
    if [ ! -e "$QNN_LIB_DIR/$qlib" ]; then
        echo "[ERROR] Missing QNN runtime lib: $QNN_LIB_DIR/$qlib"
        exit 1
    fi
done
