...
"%CL_PATH%" /std:c++20 /EHsc /W3 /O2 /utf-8 ^
  /I"%ROOT_DIR%\src\pc" ^
  "%ROOT_DIR%\tests\pc\chatbox_formatter_test.cpp" ^
  "%ROOT_DIR%\src\pc\chatbox_formatter.cpp" ^
  /Fe"%BUILD_DIR%\chatbox_formatter_test.exe" >nul
if errorlevel 1 exit /b 1

"%BUILD_DIR%\chatbox_formatter_test.exe"
exit /b %ERRORLEVEL%
