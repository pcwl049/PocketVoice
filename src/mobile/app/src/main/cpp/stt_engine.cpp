...
static int readQnnSocId(const std::string& modelDir) {
    // QNN_SOC_MODEL_* value from QnnTypes.h (SM8735 = 85, SM8750 = 69, SM8550 = 43).
    int fileValue = readPositiveIntFile(modelDir + "/qnn_soc_id.txt");
    if (fileValue > 0) return fileValue;
    const char* raw = std::getenv("STT_QNN_SOC_ID");
    if (!raw || !*raw) return 85;
    int value = std::atoi(raw);
    if (value <= 0) return 85;
    return value;
}

static std::string readQnnDspArch(const std::string& modelDir) {
    // Must match the device HTP arch (SM8735 -> v73, SM8750 -> v79, SM8550 -> v73).
    std::string fileValue = readTextFile(modelDir + "/qnn_dsp_arch.txt");
    if (!fileValue.empty()) return fileValue;
    const char* raw = std::getenv("STT_QNN_DSP_ARCH");
...
static bool writeQnnHtpConfig(const std::string& path, const std::string& modelDir) {
    ...
    int socId = readQnnSocId(modelDir);
    std::string dspArch = readQnnDspArch(modelDir);
...
bool SttEngine::init(const std::string& modelDir, const std::string& qnnLibDir) {
    ...
    if (useSenseVoiceQnn) {
        ...
        if (writeQnnHtpConfig(htpConfigPath, modelDir)) {
            ...
            LOGI("QNN HTP soc_id: %d, dsp_arch: %s", readQnnSocId(modelDir), readQnnDspArch(modelDir).c_str());
        } else {
        ...
        // Context binary path: when the file does not exist yet, sherpa runs
        // InitFromModelLib (~14s graph build) and then saves a binary via
        // CreateContextBinary using the CURRENT QNN stack; later starts load
        // it directly (fast path). A binary generated by a different QNN
        // revision fails to deserialize ("graphs Info v3"), so delete the
        // file whenever the QNN runtime stack is swapped.
