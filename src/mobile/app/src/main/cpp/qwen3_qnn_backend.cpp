...
bool Qwen3QnnBackend::init(const std::string& modelDir, const std::string& qnnLibDir) {
    ...
    LOGI("Running Gate C2: Cache Buffer Connectivity Probe");
    runCacheBufferProbe();
...
bool Qwen3QnnBackend::runCacheBufferProbe() {
    if (!m_initialized || !m_impl->decoderGraph) {
        LOGE("CacheBuf probe: Backend not initialized");
        return false;
    }

    LOGI("=== Gate C2: Cache Buffer Connectivity Probe Start ===");

    auto* gi = m_impl->graphsInfo[0];
    if (!gi) {
        LOGE("CacheBuf probe: No graph info");
        return false;
    }

    // Common inputs for all three runs
    int64_t inputIds[1] = {0};
    int64_t attentionMask[1] = {1};
    int64_t positionScalar[1] = {1};  // position>0 to unmask past KV
    std::vector<float> ropeEmb(ROPE_FREQ_DIM, 0.0f);
    for (int i = 0; i < ROPE_FREQ_DIM; ++i) {
        ropeEmb[i] = 1.0f * kRopeInvFreq[i];  // position=1
    }
    ...
    // Result struct per run
    struct ProbeResult {
        float logitsSum = 0.0f;
        int logitsArgmax = 0;
        float keyDelta0Max = 0.0f;
        float valueDelta0Max = 0.0f;
        int keyDelta0Nonzero = 0;
        int valueDelta0Nonzero = 0;
    };

    auto runOnce = [&](const float* overrideKeyData, const float* overrideValueData, ProbeResult& result) -> bool {
        std::vector<std::vector<uint8_t>> inputBuffers(gi->numInputTensors);
        std::vector<Qnn_Tensor_t> inputs(gi->numInputTensors);
        ...
        Qnn_ErrorHandle_t err = m_impl->qnnInterface.graphExecute(
            ...
            nullptr, nullptr);
        if (err != QNN_SUCCESS) {
            LOGE("CacheBuf probe: graphExecute failed: %lu", static_cast<unsigned long>(err));
            return false;
        }
        ...
        for (uint32_t i = 0; i < gi->numOutputTensors; i++) {
            ...
            if (strcmp(name, "logits") == 0) {
                std::vector<float> logitsData(elemCount, 0.0f);
                Impl::readTensorToFloat(outputs[i], outputBuffers[i].data(), logitsData.data(), elemCount);
                result.logitsSum = 0.0f;
                result.logitsArgmax = 0;
                float maxVal = logitsData[0];
                for (size_t v = 0; v < static_cast<size_t>(VOCAB_SIZE) && v < elemCount; ++v) {
                    result.logitsSum += logitsData[v];
                    if (logitsData[v] > maxVal) {
                        maxVal = logitsData[v];
                        result.logitsArgmax = static_cast<int>(v);
                    }
                }
            } else if (strcmp(name, "key_delta_0") == 0) {
                std::vector<float> deltaData(elemCount, 0.0f);
                Impl::readTensorToFloat(outputs[i], outputBuffers[i].data(), deltaData.data(), elemCount);
                for (size_t v = 0; v < elemCount; ++v) {
                    float absV = std::abs(deltaData[v]);
                    if (absV > result.keyDelta0Max) result.keyDelta0Max = absV;
                    if (absV > 1e-8f) result.keyDelta0Nonzero++;
                }
            } else if (strcmp(name, "value_delta_0") == 0) {
                std::vector<float> deltaData(elemCount, 0.0f);
                Impl::readTensorToFloat(outputs[i], outputBuffers[i].data(), deltaData.data(), elemCount);
                for (size_t v = 0; v < elemCount; ++v) {
                    float absV = std::abs(deltaData[v]);
                    if (absV > result.valueDelta0Max) result.valueDelta0Max = absV;
                    if (absV > 1e-8f) result.valueDelta0Nonzero++;
                }
            }
        }
        return true;
    ...
    // --- Run A: zero cache ---
    ProbeResult rA;
    LOGI("CacheBuf probe: Run A (zero cache)...");
    if (!runOnce(nullptr, nullptr, rA)) {
        LOGE("CacheBuf probe: Run A failed");
        return false;
    }
    LOGI("  A: logits_sum=%.2f argmax=%d key_delta0_max=%.6f nonzero=%d value_delta0_max=%.6f nonzero=%d",
         rA.logitsSum, rA.logitsArgmax, rA.keyDelta0Max, rA.keyDelta0Nonzero,
         rA.valueDelta0Max, rA.valueDelta0Nonzero);

    // --- Run B: max-pattern cache ---
    // cache_key quant range [-512, 512], scale=0.015625, uint16 max=65535
    // max dequant value = 65535 * 0.015625 = 1023.984375
    // cache_value quant range [-1, 1], scale=3.05e-5, uint16 max=65535
    // max dequant value = 65535 * 3.05e-5 = 2.0
    const size_t windowElems = static_cast<size_t>(W) * NUM_HEADS * HEAD_DIM;
    std::vector<float> maxKeyData(NUM_LAYERS * windowElems);
    std::vector<float> maxValueData(NUM_LAYERS * windowElems);
    for (int layer = 0; layer < NUM_LAYERS; ++layer) {
        std::fill(maxKeyData.begin() + layer * windowElems,
                  maxKeyData.begin() + (layer + 1) * windowElems, 1023.984375f);
        std::fill(maxValueData.begin() + layer * windowElems,
                  maxValueData.begin() + (layer + 1) * windowElems, 2.0f);
    }

    ProbeResult rB;
    LOGI("CacheBuf probe: Run B (max-pattern cache)...");
    if (!runOnce(maxKeyData.data(), maxValueData.data(), rB)) {
        LOGE("CacheBuf probe: Run B failed");
        return false;
    }
    LOGI("  B: logits_sum=%.2f argmax=%d key_delta0_max=%.6f nonzero=%d value_delta0_max=%.6f nonzero=%d",
         rB.logitsSum, rB.logitsArgmax, rB.keyDelta0Max, rB.keyDelta0Nonzero,
         rB.valueDelta0Max, rB.valueDelta0Nonzero);

    // --- Run C: realkv-like cache ---
    // Real cache_key absmax ~330, cache_value absmax ~0.9
    std::vector<float> realKeyData(NUM_LAYERS * windowElems);
    std::vector<float> realValueData(NUM_LAYERS * windowElems);
    for (int layer = 0; layer < NUM_LAYERS; ++layer) {
        for (size_t idx = 0; idx < windowElems; ++idx) {
            const int h = static_cast<int>(idx / HEAD_DIM);
            const int d = static_cast<int>(idx % HEAD_DIM);
            const float phase = static_cast<float>(layer) * 0.1f +
                                static_cast<float>(h) * 0.01f +
                                static_cast<float>(d) * 0.001f;
            realKeyData[static_cast<size_t>(layer) * windowElems + idx] =
                std::sin(phase) * 300.0f;
            realValueData[static_cast<size_t>(layer) * windowElems + idx] =
                std::cos(phase) * 0.85f;
        }
    }

    ProbeResult rC;
    LOGI("CacheBuf probe: Run C (realkv-like cache)...");
    if (!runOnce(realKeyData.data(), realValueData.data(), rC)) {
        LOGE("CacheBuf probe: Run C failed");
        return false;
    }
    LOGI("  C: logits_sum=%.2f argmax=%d key_delta0_max=%.6f nonzero=%d value_delta0_max=%.6f nonzero=%d",
         rC.logitsSum, rC.logitsArgmax, rC.keyDelta0Max, rC.keyDelta0Nonzero,
         rC.valueDelta0Max, rC.valueDelta0Nonzero);

    // --- Decision ---
    const float abDiff = std::abs(rA.logitsSum - rB.logitsSum);
    const float acDiff = std::abs(rA.logitsSum - rC.logitsSum);

    LOGI("=== Gate C2: Cache Buffer Probe Results ===");
    LOGI("  A vs B logits_sum diff: %.6f", abDiff);
    LOGI("  A vs C logits_sum diff: %.6f", acDiff);

    if (abDiff > 0.0f) {
        LOGI("  VERDICT: HTP READS cache buffers (A vs B differs).");
        LOGI("  Scale is unusable (1.53e-9) but connectivity exists.");
    } else if (acDiff > 0.0f) {
        LOGI("  VERDICT: HTP READS cache buffers with real-KV data (A vs C differs, A vs B same).");
        LOGI("  Max-pattern may not survive quantization but real values do.");
    } else {
        LOGI("  VERDICT: HTP does NOT read cache buffers, or signal is truncated below observable output.");
        LOGI("  Focus on input binding, graph connectivity, or layout.");
    }

    LOGI("=== Gate C2: Cache Buffer Probe End ===");
    return true;
}
